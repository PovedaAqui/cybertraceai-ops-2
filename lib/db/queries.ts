...
export async function createUser(email: string, name?: string) {
  try {
    const { db } = getDbConnection();
    // Note: In NextAuth setup, user IDs are generated by the auth provider
    // This function is mainly for testing or manual user creation
...
export async function saveChat(newChat: {
  ...
  try {
    ...
    const chatData = {
      ...newChat,
      id: newChat.id || `${newChat.userId}_${Date.now()}`
    };
    return await db.insert(schema.Chat).values(chatData).returning();
  } catch (error) {
...
// Get message count for a chat (for ID generation)
...
export async function saveMessages(messages: NewMessage[]) {
  try {
    ...
    const dbMessages = messages.map((msg, index) => ({
      // Generate message ID based on chat ID and sequence
      id: `${msg.chatId}_${String(index + 1).padStart(3, '0')}`,
      chatId: msg.chatId,
      role: msg.role,
      content: msg.content,
      parts: msg.parts,
